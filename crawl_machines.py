...
EXPORT_JSON = True  # Enable JSON export of results at the end
DRY_RUN = False     # If True, does not save to DB, does not mark done/failed
...
async def crawl_machines_from_catalogue(depth=0, dry_run=DRY_RUN):
    ...
    for catalogue_url in CATALOGUE_URLS:
        print(f"🌐 Reading catalog: {catalogue_url}")
        try:
            r = httpx.get(catalogue_url, headers={"User-Agent": "Mozilla/5.0"}, follow_redirects=True)
            html = r.text
            sub_links = extract_machine_links(html)
            logger.info(f"🔗 {len(sub_links)} links extracted from {catalogue_url}")
            links.extend(sub_links)
        except Exception as e:
            print(f"❌ Catalog error {catalogue_url} → {e}")

    links = list(set(links))  # deduplicated
    ...
    logger.info(f"📥 {len(links)} URL candidates, {len([u for u in links if not is_ignored_url(u) and not queue.is_known(u)])} added to the queue.")
            ...
            if response and response.status_code == 200:
                ...
                if page_type in ("machine", "promo", "service"):
                    ...
                    if data and "title" in data:
                        ...
                        print(f"⚠️ No data saved for {url}")
                        n_failed += 1
                        if not dry_run:
                    ...
                    logger.warning(f"Unknown page type for {url}")
                    n_failed += 1
                    if not dry_run:
                        queue.mark_failed(url, reason="unknown type")
                    else:
                        logger.info(f"[DRY RUN] {url} would have been marked as failed (unknown type).")
            else:
    ...
    logger.info(f"Final queue summary: {queue.count_by_status()}")
    logger.info(f"Crawl finished in {elapsed:.1f} seconds. Success: {n_success}, Failures: {n_failed}, Pages/sec: {n_success/elapsed if elapsed else 0:.2f}")

    if EXPORT_JSON:
        with open("crawl_results.json", "w") as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        logger.info(f"JSON export finished: crawl_results.json")

async def retry_failed_urls(max_retries=2, batch_size=5):
    """
    Automatically retries failed URLs (status 'failed'), up to max_retries times.
    """
    queue = URLQueue()
